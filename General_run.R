...
segmented_pastdata<-segment_data(pastdata,m=50000)
...
#             "issue_d", "total_rec_int", "total_rec_late_fee", "next_pymnt_d", "tot_cur_bal", "installment", "loan_status","payment_gap")
# 

picklist<-c("num_sats","total_il_high_credit_limit","emp_length","total_rec_late_fee","last_pymnt_d",
          "last_pymnt_amnt","dti","issue_d","int_rate","sub_grade", "term", "annual_inc","avg_cur_bal",
          "loan_status","payment_gap")
...
customerscreeningmodel<-model_run(model_data,"rf",ntrees=150,importance=TRUE)

#Save the model
save(customerscreeningmodel, file = "RF_model_for_customer_Screening_m_100000.rda")

#Run the machine learning algorithm for past data and use this information to evaluate current customer base
system.time(currentcustomermodel<-model_run(segmented_pastdata,"rf",ntrees=150,importance=TRUE))

#Save the model
save(currentcustomermodel, file = "RF_model_for_current_customer_Screening_m_test.rda")